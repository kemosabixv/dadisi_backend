...
{
    public function __construct(private RenewalReminderServiceContract $reminderService)
    {
    ...
    public function requestManualRenewal(Request $request, $subscriptionId): JsonResponse
    {
        try {
            ...
            // Manual renewal request workflow
            // Real payment handling will be in PaymentController or via Pesapal redirect.
            ...
            Log::error('Manual renewal request failed', [
                'error' => $e->getMessage(),
    ...
    public function getManualRenewalOptions(Request $request, $subscriptionId): JsonResponse
    {
        try {
            ...
            Log::error('Failed to fetch renewal options', [
                'error' => $e->getMessage(),
                'subscription_id' => $subscriptionId,
                'user_id' => $request->user()?->id
            ]);
            return response()->json(['success' => false, 'message' => 'Failed to fetch renewal options'], 500);
        }
    }
    ...
    public function confirmManualRenewal(Request $request, $subscriptionId): JsonResponse
    {
        try {
            ...
            Log::error('Failed to confirm renewal', [
                'error' => $e->getMessage(),
                'subscription_id' => $subscriptionId,
                'user_id' => $request->user()?->id
            ]);
            return response()->json(['success' => false, 'message' => 'Failed to confirm renewal'], 500);
        }
    }
    ...
    public function getPendingReminders(Request $request): JsonResponse
    {
        try {
            ...
            Log::error('Failed to fetch pending reminders', [
                'error' => $e->getMessage(),
                'user_id' => $request->user()?->id
            ]);
    ...
    public function extendGracePeriod(Request $request, $subscriptionId): JsonResponse
    {
        try {
            ...
            Log::error('Failed to extend grace period', [
                'error' => $e->getMessage(),
                'subscription_id' => $subscriptionId,
                'user_id' => $request->user()?->id
            ]);
            return response()->json(['success' => false, 'message' => 'Failed to extend grace period'], 500);
        }
    }
}
