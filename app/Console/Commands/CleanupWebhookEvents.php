...
use App\Jobs\CleanupWebhookEventsJob;
...
class CleanupWebhookEvents extends Command
{
     ...
     * @var string
     */
    protected $signature = 'webhooks:cleanup-events {--sync} {--dry-run} {--days=}';
     ...
     * @var string
     */
    protected $description = 'Delete old webhook event records based on retention settings';
    ...
    public function handle(): int
    {
        try {
            ...
            $this->info("🗑️  Webhook Events Cleanup Command");
            $this->info("{'━'━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━}");
            ...
            if ($sync || $dryRun) {
                ...
                app()->call([new CleanupWebhookEventsJob($retentionDays, $dryRun), 'handle']);
                $this->info("✅ Cleanup job completed");

                if ($dryRun) {
                    $this->line("<fg=yellow>Note: This was a dry-run. No data was deleted.</>");
                }
            } else {
                // Queue job for async execution
                $this->info("📤 Dispatching cleanup job to queue...");
                CleanupWebhookEventsJob::dispatch($retentionDays, $dryRun);
                $this->info("✅ Job dispatched to queue for processing");
            }

            Log::info('CleanupWebhookEvents command executed', [
                'retention_days' => $retentionDays,
                'sync' => $sync,
                'dry_run' => $dryRun,
            ]);
            ...
            Log::error('CleanupWebhookEvents command failed', [
                'error' => $e->getMessage(),
    ...
    {
        $setting = UserDataRetentionSetting::where('data_type', 'webhook_events')
